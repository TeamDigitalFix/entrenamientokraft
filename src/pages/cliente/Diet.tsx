...
const ClientDiet = () => {
  ...
  const { toggleMealTypeCompletion, isToggling } = useMealToggle();
  ...
  const areMealsCompleted = (meals: typeof diet.mealsByDay[string]) => {
    if (!meals.length) return false;
    return meals.every(meal => meal.completed);
  };

  // Handler para marcar/desmarcar un tipo de comida completo
  const handleToggleMealType = (meals: typeof diet.mealsByDay[string], isCompleted: boolean) => {
                                ...
                                <div className="flex justify-between items-center mb-3">
                                  <div className="flex items-center space-x-3">
                                    <Button
                                      ...
                                      disabled={isToggling}
                                      onClick={() => handleToggleMealType(meals, isCompleted)}
                                    >
                                      {isCompleted ? (
                                        <>
