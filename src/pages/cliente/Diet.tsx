...
const ClientDiet = () => {
  const { diet, isLoading, activeDate, setActiveDate, availableDates } = useClientDiet();

  // Group meals by type for the active date
  const getMealsByType = (date: string) => {
    if (!diet?.mealsByDate[date]) return {};
    
    const mealsByType: Record<string, typeof diet.mealsByDate[string]> = {};
    
    diet.mealsByDate[date].forEach(meal => {
  ...
  const calculateMealTypeMacros = (meals: typeof diet.mealsByDate[string]) => {
    return meals.reduce(
      (acc, meal) => {
          ...
          <CardContent className="pt-6">
            {isLoading ? (
              ...
              <div className="space-y-4">
                ...
                {activeDate && diet.mealsByDate[activeDate]?.length > 0 ? (
                  <div className="space-y-6">
                    {Object.entries(getMealsByType(activeDate)).map(([mealType, meals]) => (
                  ...
                  <div className="text-center py-8">
                    <Utensils className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
                    <p className="text-muted-foreground">No hay comidas programadas para esta fecha</p>
                  </div>
                )}
              </div>
            )}
